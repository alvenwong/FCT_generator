...
void load_cdf(struct cdf_table *table, const char *file_name)
{
    ...
    while (fgets(line, sizeof(line), fd))
    {
		...
		if (table->entries[table->num_entry].cdf > MAX_CDF) {
			printf("The cdf entry is larger than 1. Please check %s.\n", file_name);
			exit(0);
		}
...
void get_cdf(struct cdf_table* table, const char* cdf_filename)
{
	init_cdf(table);
	load_cdf(table, cdf_filename);
}
